...
class PreviewFinder {
  ...
  activate() {
    ...
    this.subscriptions.add(
      commands.add(document.body, {
        "preview-finder:find": this.openFinder,
  ...
  findLoop = (mode) => {
    ...
    console.log(`this.noteId = ${this.noteId}, current = ${id}`);
    if (this.noteId != id) {
      this.noteId = id;
      this.findNodes();
    }
    ...
    console.log(`paneTop:${paneTop}, paneHeight:${pane.clientHeight} top:${top}`);
    if (top < paneTop || paneTop + pane.clientHeight < top) {
      node.scrollIntoView();
      pane.scrollTop -= pane.clientHeight / 2;
    }
    console.log(`top: ${top}, pane.scrollTop: ${pane.scrollTop}`);
